...
const DiscardedItems = () => {
    ...
    useEffect(() => {
        axios.get('http://localhost:5000/api/discardedItems')
...
const addDiscardedItems = (e) => {
    ...
    axios.post('http://localhost:5000/api/discardedItems', newDiscardedItems)
    .then(response => {
    ...
    return (
        ...
        <ul>
            ...
            <li key={index}>{item.sku} - {item.quantity} - {item.reason} - {item.discarddate}</li>
            ))}
        </ul>
